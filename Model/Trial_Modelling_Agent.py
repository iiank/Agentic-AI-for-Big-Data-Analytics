...
class AgentState(TypedDict):
    ...
    df:                    object           # engineered Spark DataFrame
    ...
    evaluation_decision:   Dict[str, Any]   # LLM Call 2: winner + narrative
...
def build_model_selection_context(state: AgentState) -> Dict:
    ...
    return {
        ...
        "spark_mode":         "local[*] on MacBook Air M2 — constrain each model's grid to 4-6 combinations",
        "available_models": {
            name: {
                "task":           info["task"],
...
while modelling_agent.get_state(config).next:
    ...
    if approval == "y":
        ...
        os = input("Override secondary model (or press Enter to keep): ").strip()
        resume = {"approved": False, "override_primary": op, "override_secondary": os}
